...
/* This file is part of ELF support for BFD, and contains the portions
   ...
   the elf/common.hpp file which contains the portions that are common to
...
/* Relocation Entries */
typedef struct {
	unsigned char rz_offset[4]; /* Location at which to apply the action */
	unsigned char rz_info[4]; /* index and type of relocation */
} Elf32_External_Rel;

typedef struct {
	unsigned char rz_offset[4]; /* Location at which to apply the action */
	unsigned char rz_info[4]; /* index and type of relocation */
	unsigned char rz_addend[4]; /* Constant addend used to compute value */
} Elf32_External_Rela;

typedef struct {
	unsigned char rz_offset[8]; /* Location at which to apply the action */
	unsigned char rz_info[8]; /* index and type of relocation */
} Elf64_External_Rel;

typedef struct {
	unsigned char rz_offset[8]; /* Location at which to apply the action */
	unsigned char rz_info[8]; /* index and type of relocation */
	unsigned char rz_addend[8]; /* Constant addend used to compute value */
} Elf64_External_Rela;
